...
> 前置知识：[[03-启动代码与链接]]。本节对应的完整工程位于 `steps/step-0-minimal/`。
...
## 源码整合：step-0-minimal/Makefile

`steps/step-0-minimal/Makefile` 是完整版本（含跨平台删除命令）：
...
> 后续 step 的 Makefile 会随着新增源文件（`startup.c`、`syscalls.c`、CMSIS 头文件等）而演化，详见 [[08-printf重定向]]、[[10-CMSIS]]、[[12-Web服务器]]。

## 下一步

固件已经能构建和烧写，但它什么也不做。接下来让它点亮 LED，见 [[05-LED闪烁]]。
