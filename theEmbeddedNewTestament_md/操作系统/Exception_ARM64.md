...
tags:
  - 操作系统
source: https://github.com/theEmbeddedNewTestament/theEmbeddedNewTestament.github.io/tree/main/Operating_System/Exception_ARM64.md
created: 2026-08-27
---

# ARM64 内核异常(exception)模式如何工作
### ARM aarch32 的异常向量表(Exception Vector Table)
![Exception_Vectors.jpg](http://kcchao.wdfiles.com/local--files/arm/Exception_Vectors.jpg)


### ARM aarch64 的异常向量表(Exception Vector Table)
...
### 以下源代码来自 Linux ARM64 3.18.102。

##### 当硬件产生异常时，内核尝试在向量表(vector table)中找到入口。
...
##### 举例来说，当发生 el0 同步中止(synchronous abort)时，内核搜索 el0_syn 入口并找到匹配的处理程序。
...
##### 举例来说，假设触发了数据中止(data abort)。内核找到数据中止处理并调用 do_mem_abort 函数。LR 寄存器被写入 ret_to_user 函数。因此当 do_mem_abort 函数完成时，会调用 ret_to_user 函数。
```C
el0_da:
	/*
	 * Data abort handling
...
##### 对于数据中止，在 struct fault info 中定义了各种故障处理函数。内核可以根据 esr 寄存器中的值，依据 struct fault info 的地址加上偏移来调用相应的函数。
...
##### 如果内核无法找到任何匹配的故障处理函数，就会发生未处理故障(unhandled fault)，内核会使机器崩溃。这种行为可以被修改，使机器得以存活。
